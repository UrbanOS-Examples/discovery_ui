...
describe('api example', () => {
  ...
  it('creates a curl command for each example', () => {
    ...
    expect(curlDivs.at(0).text()).toBe("curl -X POST 'localhost:5000/api' -H 'Content-Type: text/plain' -d 'select * from thing'")
    expect(curlDivs.at(1).text()).toBe("curl -X POST 'localhost:5000/api' -H 'Content-Type: text/plain' -d 'select * from bob join bobi using (bob_id)'")
  })
})
